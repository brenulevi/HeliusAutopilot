...
int main(void)
{
  ...
  while (1)
  {
    ...
    {
      printf("Accel: X=%.4f, Y=%.4f, Z=%.4f | Gyro: X=%.4f, Y=%.4f, Z=%.4f | Temp: %.2f\n",
             mpu9250.data.accel_mps2.x, mpu9250.data.accel_mps2.y, mpu9250.data.accel_mps2.z,
             mpu9250.data.gyro_rps.x, mpu9250.data.gyro_rps.y, mpu9250.data.gyro_rps.z,
             mpu9250.data.temperature_c);
    }
    else
    {
      printf("Failed to read data from MPU9250!\n");
    }
    /* USER CODE END WHILE */
